...
do
  ...
  then
    ...
    iptables -w -t nat -p tcp -D PREROUTING -s $1 --match multiport --dports $puerto -j DNAT --to-destination $ip

  else
    echo "Eliminando puerto singular";
    iptables -w -t nat -p tcp -D PREROUTING -s $1 --dport $puerto -j DNAT --to-destination $ip
  fi
